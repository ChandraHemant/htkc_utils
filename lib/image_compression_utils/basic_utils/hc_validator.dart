...
import 'package:htkc_utils/image_compression_utils/basic_utils/hc_compress_format.dart';

class HcValidator {
  final MethodChannel channel;

  HcValidator(this.channel);

  bool ignoreCheckExtName = false;
  bool ignoreCheckSupportPlatform = false;

  void checkFileNameAndFormat(String name, HcCompressFormat format) {
    if (ignoreCheckExtName) {
      return;
    }
    name = name.toLowerCase();
    if (format == HcCompressFormat.jpeg) {
      assert((name.endsWith(".jpg") || name.endsWith(".jpeg")),
          "The jpeg format name must end with jpg or jpeg.");
    } else if (format == HcCompressFormat.png) {
      assert(name.endsWith(".png"), "The jpeg format name must end with png.");
    } else if (format == HcCompressFormat.heic) {
      assert(
          name.endsWith(".heic"), "The heic format name must end with heic.");
    } else if (format == HcCompressFormat.webp) {
      assert(
          name.endsWith(".webp"), "The webp format name must end with webp.");
    }
  }

  Future<bool> checkSupportPlatform(HcCompressFormat format) async {
    if (ignoreCheckSupportPlatform) {
      return true;
    }
    if (format == HcCompressFormat.heic) {
      if (Platform.isIOS) {
        ...
        const msg = "The hectic format only support iOS 11.0+";
        assert(result, msg);
