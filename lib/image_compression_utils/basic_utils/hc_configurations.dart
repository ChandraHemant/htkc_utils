...
class HcConfiguration {
  ...
  /// The [HcImageOutputType] of image file
  final HcImageOutputType outputType;

  /// True if using native platform compressor for JPG or PNG
  final bool useJpgPngNativeCompressor;

  const HcConfiguration({
    this.quality = 80,
    this.outputType = HcImageOutputType.webpThenJpg,
    this.useJpgPngNativeCompressor = false,
  }) : assert(quality > 0 && quality <= 100);
}

/// The wrapper object for [ImageFile] and [HcConfiguration]
class HcImageFileConfiguration {
  HcImageFileConfiguration({
    required this.input,
    this.config = const HcConfiguration(),
  });
