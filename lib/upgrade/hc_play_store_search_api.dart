...
class HcPlayStoreSearchAPI {
  HcPlayStoreSearchAPI({http.Client? client}) : client = client ?? http.Client();
...
extension PlayStoreResults on HcPlayStoreSearchAPI {
  ...
  String? redesignedVersion(Document response) {
    ...
    try {
      ...
      if (debugLogging) {
        if (kDebugMode) {
          print('hcUpgrade: PlayStoreResults.redesignedVersion exception: $e');
        }
      }
    }
